...
-(id) init {
    ...
    */
    ...
    labelBottom = [CCLabelBMFont labelWithString:delegate.currentPageDesc fntFile:@"bottom-menu-14-hd.fnt"];
	...
	htmlView = [[UIWebView alloc] initWithFrame:CGRectMake(0.0, 52.0*2, size.width, size.height-52.0*2-47.0*2)];
    ...
    NSString *setTextSizeRule = [NSString stringWithFormat:@"addCSSRule('body', '-webkit-text-size-adjust: %d%%;')",32];
    [htmlView stringByEvaluatingJavaScriptFromString:setTextSizeRule];
    ...
    for (int i=0; i < 8; i++) {
        ...
        CCSprite *aDot = [CCSprite spriteWithFile:@"dot-hd.png"];
        aDot.position = ccp(280.0+28.0*i,37.0*2);
        if (i > 0)
            aDot.opacity = 128;
...
- (void) handleSwipeGestureLeft: (id) sender
{
    ...
    if ([delegate.currentPage isEqualToString:@"Intro"]) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            htmlView.frame = CGRectMake(neutral-768, htmlView.frame.origin.y, htmlView.frame.size.width, htmlView.frame.size.height);
        }];
...
- (void)dealloc {
	...
	NSLog(@"releasing ContentScene elements");
